...
import { getApiBaseUrl, getBackendUrl, withNgrokHeaders } from "@/lib/config";
...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    ...
    return NextResponse.redirect(
      new URL(`/home?error=${error}`, "https://chancifyaipresidential.up.railway.app"),
    );
  }

  if (!code) {
    return NextResponse.redirect(
      new URL("/home?error=no_code", "https://chancifyaipresidential.up.railway.app"),
    );
  }
  ...
  try {
    ...
    if (!clientId || !clientSecret) {
      ...
      return NextResponse.redirect(
        new URL("/home?error=missing_config", "https://chancifyaipresidential.up.railway.app"),
      );
    }
    ...
    try {
      ...
      const createUserUrl = getBackendUrl("/api/auth/google-oauth");
    ...
    return NextResponse.redirect(
      new URL("/home?error=oauth_failed", "https://chancifyaipresidential.up.railway.app"),
    );
  }
}
