...
export default function CorrectedGoogleOAuth() {
  ...
  const handleSignIn = () => {
    ...
    // Simulate callback handling until you implement backend
    const checkPopupClosed = setInterval(() => {
      if (popup.closed) {
        clearInterval(checkPopupClosed)
        setIsLoading(false)
        
        // Store demo user data for now
        const userData = {
  ...
  return (
    <button
      ...
      disabled={isLoading}
      className="w-full bg-white hover:bg-gray-100 disabled:bg-white/60 text-black font-semibold py-3 rounded-xl border border-gray-300 shadow-sm transition-all duration-200 flex items-center justify-center gap-3"
    >
